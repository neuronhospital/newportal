...
function doPost(e) {
  try {
    ...
    switch (action) {
      ...
      case "validateRetrievalSession":
        return json_(validateRetrievalSession_(body));
  ...
  return {
    ...
    patients: todayRows.map(function(found) {
      ...
      return {
        ...
        patientType: row[6],
        reason: "",
        address: clean_(row[5] == null ? "" : row[5]),
        referredBy: clean_(row[9] == null ? "" : row[9]),
        whatsapp: normalizePhone_(row[7]),
        opdCharges: row[11],
        opdPaymentMode:
          Number(opdPayment.cashPaid)>0 && Number(opdPayment.onlinePaid)>0 ? "Split" :
          Number(opdPayment.onlinePaid)>0 ? "Online" :
          Number(opdPayment.cashPaid)>0 ? "Cash" : "Free",
        opdCashPaid: opdPayment.cashPaid,
        opdOnlinePaid: opdPayment.onlinePaid,
        opdTotalPaid: opdPayment.totalPaid,
        eegCharges: row[14],
...
function validateRetrievalSession_(data) {
  ...
  sheets.forEach(function(sheet) {
    ...
    function readBlock(startRow, endRow) {
      ...
      block.forEach(function(row) {
        ...
        const opdPaymentMode =
          Number(opdPay.cashPaid)>0 && Number(opdPay.onlinePaid)>0 ? "Split" :
          Number(opdPay.onlinePaid)>0 ? "Online" :
          Number(opdPay.cashPaid)>0 ? "Cash" : "Free";

        rows.push({
          patientName: patientName,
          age: row[4],
          ageUnit: "",
          address: clean_(row[5]),
          opdPaymentMode: opdPaymentMode,
