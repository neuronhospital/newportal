...
document.addEventListener("DOMContentLoaded",()=>{
 ...
 const opdHasChanges=(candidate)=>{
   const original=selected||{};
   const same=(a,b)=>normalizedText(a)===normalizedText(b);
   const sameNumber=(a,b)=>Number(a??0)===Number(b??0);
   return !(same(U.title($('name').value),original.name||'')&&sameNumber($('age').value,original.age)&&same($('unit').value,original.ageUnit||'years')&&same(U.title($('address').value),original.address||'')&&same(U.title($('ref').value),original.referredBy||'')&&same(U.phone($('editWa').value),U.phone(original.whatsapp||''))&&same($('nextFollowupCity').value,original.nextFollowupCity||'')&&sameNumber(candidate.charge,original.totalOPDCharges)&&sameNumber(candidate.cash,original.opdCashPaid)&&sameNumber(candidate.online,original.opdOnlinePaid));
 };
 ...
 $('save').onclick=async()=>{
   ...
   const candidate={charge,cash,online};
   if(!opdHasChanges(candidate)){alert("No changes were made. Update was not done.");return;}
