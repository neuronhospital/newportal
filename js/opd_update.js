...
document.addEventListener("DOMContentLoaded",()=>{
 ...
 $('save').onclick=async()=>{
   ...
   try{const r=await NeuronAPI.call("updateOPDDetails",p,25000);const before=r.before||{},after=r.after||{};const fields=[['Patient Name',before.name,after.name],['Age',`${before.age} ${before.ageUnit}`,`${after.age} ${after.ageUnit}`],['Address',before.address,after.address],['Referred By Dr./Hospital',before.referredBy,after.referredBy],['WhatsApp Number',before.whatsapp,after.whatsapp],['Next Follow-up City',before.nextFollowupCity,after.nextFollowupCity],['OPD Charges Paid',before.opdTotalPaid,after.opdTotalPaid],['Paid in Cash',before.opdCashPaid,after.opdCashPaid],['Paid Online',before.opdOnlinePaid,after.opdOnlinePaid]];const changed=fields.filter(f=>String(f[1]??"")!==String(f[2]??""));$('confirmation').hidden=false;$('confirmation').innerHTML=`<div class="success"><div class="success-icon">✓</div><h2>OPD Details Updated</h2><p>Appointment ID: <b>${U.esc(r.appointmentId)}</b></p>${changed.length?`<p><b>Changed and updated:</b></p><ul class="changed-list">${changed.map(f=>`<li><b>${U.esc(f[0])}</b>: ${U.esc(String(f[1]??""))} → <b>${U.esc(String(f[2]??""))}</b></li>`).join('')}</ul>`:`<p>No values were changed.</p>`}</div>`;selected=null;$('patients').innerHTML="";$('edit').hidden=true;$('name').value="";$('age').value="";$('address').value="";$('ref').value="";$('editWa').value="";$('nextFollowupCity').innerHTML="";$('charge').value="";$('cash').value="";$('online').value="";$('cash').dataset.actual="";$('online').dataset.actual="";$('totalPaid').textContent="0";}catch(e){
     ...
     if(uncertain){
       $('saveStatus').hidden=false;$('saveStatus').style.color='#8a4b00';$('saveStatus').textContent='Unable to confirm the update. Checking whether it was recorded...';
       let recovered=null;
       try{recovered=await NeuronAPI.verifyUpdate({requestId:requestId,updateType:'OPD',appointmentId:p.appointmentId,rowNumber:p.rowNumber,city:p.city,whatsapp:p.whatsapp,name:p.name,age:p.age,ageUnit:p.ageUnit,address:p.address,referredBy:p.referredBy,nextFollowupCity:p.nextFollowupCity,whatsappNew:p.whatsappNew,opdCharges:p.opdCharges,opdCashPaid:p.opdCashPaid,opdOnlinePaid:p.opdOnlinePaid});}catch(_){}
       if(recovered&&recovered.found){
         const after=recovered.patient||{};
         $('confirmation').hidden=false;$('confirmation').innerHTML=`<div class="success"><div class="success-icon">✓</div><h2>OPD Details Updated</h2><p>Appointment ID: <b>${U.esc(recovered.appointmentId||p.appointmentId)}</b></p><p>Update was successfully recorded. Current OPD charges: <b>${U.money(after.opdTotalPaid)}</b></p></div>`;
         $('saveStatus').textContent='✓ Update confirmed successfully.';$('saveStatus').style.color='#168a4a';selected=null;$('patients').innerHTML='';$('edit').hidden=true;
       }else{$('saveStatus').textContent='We could not confirm the update. Please do not submit it again immediately; reload the patient and verify the current details.';}
     }else{$('saveStatus').hidden=false;$('saveStatus').style.color='#b42318';$('saveStatus').textContent=e.message||"Unable to update OPD details.";}
