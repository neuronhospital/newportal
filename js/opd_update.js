...
function patchOPDUpdateToday_(p){return IDB.patchTodayPatient_({appointmentId:p.appointmentId,city:p.city,date:p.appointmentDate,patch:{name:p.name,age:p.age,ageUnit:p.ageUnit,address:p.address,referredBy:p.referredBy,whatsapp:p.whatsappNew||p.whatsapp,nextFollowupCity:p.nextFollowupCity,opdCharges:p.opdCharges,totalOPDCharges:p.opdCharges,opdCashPaid:p.opdCashPaid,opdOnlinePaid:p.opdOnlinePaid}});}
...
document.addEventListener("DOMContentLoaded",()=>{
 ...
 // Confirmation-box display only. Existing payment logic, payloads and transaction mode are untouched.
 ...
 ['charge','cash','online'].forEach(id=>$(id)?.addEventListener('click',e=>{if(e.currentTarget.readOnly)showChargeLock();}));
