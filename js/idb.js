window.IDB={
 ...
 getTodayPatientsByWhatsApp_({city,whatsapp}={}){
  ...
  const phone=this.normalizeWhatsApp_(whatsapp);
  const date=this.todayKey_();
  if(!c||!/^[0-9]{10}$/.test(phone))return Promise.resolve({source:"invalid",patients:[],cache:null});
  return this.findTodayPatientsByWhatsApp_({city:c,whatsapp:phone,date}).then(async local=>{
   ...
   if(!local.cache || this.cacheStale_(local.cache,local.cache.patients,"appointmentId") || local.cache.status==="CACHED_INCOMPLETE" || local.cache.status==="STALE"){
     const cache=await this.getTodayOPDCache_(c,{forceRefresh:true});
     const patients=Array.isArray(cache?.patients)?cache.patients.filter(p=>String(p?.city||c).trim()===c&&this.normalizeWhatsApp_(p?.whatsapp)===phone):[];
     return {source:patients.length?"server":"idb-refresh",patients,cache,todayAppointmentFound:Array.isArray(cache?.patients)&&cache.patients.some(p=>String(p?.city||c).trim()===c)};
   }
   ...
   if(!serverState){
   ...
   }else if(!cache||!Number.isInteger(Number(cache.lastSerial))||!Number.isInteger(Number(cache.lastRowNumber))){
     cache=await this.getTodayOPDCache_(c,{forceRefresh:true});
   }else if(Number(cache.lastSerial)===Number(serverState.serial)&&Number(cache.lastRowNumber)===Number(serverState.rowNumber)){
     const found=Array.isArray(cache.patients)&&cache.patients.some(p=>String(p?.city||c).trim()===c);
     return {source:"idb",patients:[],cache:cache,todayAppointmentFound:found};
   }else{
     syncResult=await window.NeuronAPI.call("getTodayOPDIncremental",{city:c,date,lastSerial:Number(cache.lastSerial),lastRowNumber:Number(cache.lastRowNumber)},25000);
     if(!syncResult||syncResult.ok!==true||syncResult.valid!==true){
       ...
       cache={...cache,patients:merged,status:"REFRESHED",complete:true,lastServerRefreshAt:Date.now(),lastServerCheckAt:Date.now(),cachedAt:cache.cachedAt||Date.now(),lastSerial:Number(serverState.serial),lastRowNumber:Number(serverState.rowNumber)};
       await this.replace("cache",cache.key,cache);
     }
   }
   const patients=Array.isArray(cache?.patients)?cache.patients.filter(p=>String(p?.city||c).trim()===c&&this.normalizeWhatsApp_(p?.whatsapp)===phone):[];
   const todayAppointmentFound=Array.isArray(cache?.patients)&&cache.patients.some(p=>String(p?.city||c).trim()===c);
   return {source:patients.length?"idb-sync":"idb",patients,cache,todayAppointmentFound};
  });
 },
 followupVisitKey_(city,rowNumber){return `VISIT|${String(city||"").trim()}|${Number(rowNumber)}`;},
  ...
  async patchTodayPatient_(data={}){
    ...
    const cache=await this.get("cache",key).catch(()=>null);
    if(!cache||!Array.isArray(cache.patients))return {updated:false,reason:"cache_missing"};
    const patients=cache.patients.slice();
    const i=patients.findIndex(x=>String(x?.appointmentId||"").trim()===appointmentId);
    if(i<0)return {updated:false,reason:"patient_missing"};
    ...
    patients[i]=next;
    const now=Date.now();
    const updated={...cache,patients,cacheUpdatedAt:now};
