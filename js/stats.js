document.addEventListener("DOMContentLoaded",async()=>{
 ...
 const scheduleStateKey="stats_schedule_default_v1";
 let scheduledCities=[];
 $("city").innerHTML=NEURON_CONFIG.cities.map(x=>`<option>${U.esc(x)}</option>`).join("");

 async function applyScheduleAwareCity(){
   const todayKey=`${q.y}-${String(q.m).padStart(2,"0")}-${String(q.d).padStart(2,"0")}`;
   try{
     const r=await NeuronAPI.call("getScheduledCitiesForDate",{year:q.y,month:q.m,day:q.d},10000);
     const entries=Array.isArray(r.cities)?r.cities:[];
     scheduledCities=entries.map(x=>x.city).filter(Boolean);
     if(scheduledCities.length){
       const now=new Date();
       ...
       const getPart=t=>Number(parts.find(x=>x.type===t)?.value||0);
       const minutes=getPart("hour")*60+getPart("minute");
       const active=entries.find(x=>
         minutes>=Number(x.start)*60 && minutes<Number(x.end)*60
       );
       const preferred=(active&&active.city)||scheduledCities[0];
       if(NEURON_CONFIG.cities.includes(preferred)) $("city").value=preferred;
       localStorage.setItem(scheduleStateKey,JSON.stringify({
         date:todayKey,
         cities:scheduledCities,
         selected:preferred,
     ...
     try{
       const cached=JSON.parse(localStorage.getItem(scheduleStateKey)||"null");
       if(cached&&cached.date===todayKey&&Array.isArray(cached.cities)&&cached.cities.length){
         scheduledCities=cached.cities;
         if(NEURON_CONFIG.cities.includes(cached.selected)) $("city").value=cached.selected;
         return;
       }
     }catch(_){}
   }
   // No scheduled visit today (or no usable schedule response):
   // retain the configured first city as the manual-selection default.
 }
 await applyScheduleAwareCity();
 $("period").innerHTML=
