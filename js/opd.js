document.addEventListener("DOMContentLoaded",()=>{
  ...
  $("waBtn").onclick=()=>{
    ...
    $("waStatus").textContent="✓ WhatsApp link opened";
    $("waStatus").style.color="#168a4a";
    enableAfterWhatsApp();
  };

  async function getCalendarDates(year,month,city){
    const key=`schedule:${city}:${year}-${String(month).padStart(2,"0")}`;
    try{
      const r=await NeuronAPI.call("getAvailableDates",{city,year,month},12000);
      await IDB.put("cache",{key,value:{dates:r.dates||[],scheduleVersion:r.scheduleVersion||""}}).catch(()=>{});
      return r.dates||[];
    }catch(_){
      const cached=await IDB.get("cache",key).catch(()=>null);
      return cached?.value?.dates||[];
    }
  }
