...
function eegHasChanges(cash,online){return !(Number(cash??0)===Number(originalPayment.cash??0)&&Number(online??0)===Number(originalPayment.online??0))}
...
$('save').onclick=async()=>{
  ...
  if(!eegHasChanges(cash,online)){alert('No changes were made. Update was not done.');return;}
